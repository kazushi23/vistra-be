...
A RESTful API backend for managing documents (folders and files) with support for file uploads and validation.
...
gep-fe/
├── config/
│   └── config.ts           # Config setup for development | production | testing environment
├── controllers/
│   └── document/
│       └── documentController.tsx    # Get list with pagination and search and sort
│   └── file/
│       └── fileController.tsx   # Create files > receive form data files and create
│   └── folder/
│       └── folderController.tsx # Create folders > receive folder name and create
├── entity/
│   └── Document.ts              # Document entity for file and folders
├── middleware/
│   └── errorHandler.ts     # Error handling for controllers to call next(error)
├── routes/
│   └── api/                     # Holds all api routes
│       ├── documentApi.ts       # Document Api GET /
│       ├── fileApi.ts           # File Api POST /create
│       ├── folderApi.ts         # Folder Api POST /create
│       └── index.ts             # Holds all api {/document, /file, /folder} 
│   └── index.ts                 # Holds root api route > /api/v1
├── service/
│   └── document/
│       └── documentService.tsx    # Get list with pagination and search and sort
│   └── file/
│       └── fileService.tsx   # Validate and extract file metadata then create
│   └── folder/
│       └── folderService.tsx    # Create folders 
│   └── baseService.ts      # Base get and create implementation to be reused
├── types/
│   └── dto/           # DTO
│       └── document/                   # Document related DTO
│           ├── document.d.ts           # Get and create types
│           └── document.dto.ts         # Get and create DTO 
│   ├── base.d.ts                       # Base DTO to be reused
...
├── index.ts            # Server + DB initialisation point
├── test/
│   └── index.test.ts   # API endpoint test
```
---
## Features
...
- **Add a new file**: Implementation mainly in fileService. Multi-file upload (up to 10 files) with drag-and-drop support, file type validation and file size validation
- **Search**: Implementation mainly in DocumentController, DocumentService and BaseService
- **Pagination**: Implementation mainly in DocumentController, DocumentService and BaseService
...
- **Real-time Validation**: Server-side checks before DB operations
...
### To make this system more complete
